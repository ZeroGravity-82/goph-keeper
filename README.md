...
4. После успешной регистрации сервер сразу открывает пользовательскую сессию и возвращает клиенту access-токен, refresh-токен и пользовательскую соль.
...
3. Сервер проверяет учетные данные и возвращает клиенту access-токен, refresh-токен и пользовательскую соль.
4. Клиент использует access-токен для последующих запросов к серверу.
...
Перед шифрованием клиент формирует JSON payload в зависимости от типа записи. Сервер не интерпретирует содержимое этого JSON и хранит только зашифрованный blob.

- `credential`: логин и пароль;
- `text`: текстовое содержимое;
- `card`: номер карты, имя владельца, срок действия, CVC/CVV;
...
`encrypted_size` передается клиентом и используется сервером для проверки полноты загрузки. Во время приема стрима сервер считает фактический размер полученных чанков и после завершения загрузки сравнивает его с заявленным `encrypted_size`. При совпадении загрузка считается успешной, а запись файла получает статус `uploaded`. При несовпадении загрузка считается неуспешной, а запись файла получает статус `failed`.
...
Скачивание файла доступно только для бинарной записи, у которой загрузка файла завершилась успешно и статус файла равен `uploaded`. Это защищает клиента от получения неполного объекта, который мог остаться после неуспешной загрузки.
...
Для локальной разработки в репозитории хранится пример конфигурационного файла `config/local.yaml`. В нем задаются несекретные настройки локального окружения. Секретные значения передаются отдельно через переменные окружения.
...
go run ./cmd/server -c config/local.yaml
```
...
- создание обычной записи пользователя;
- создание бинарной записи пользователя вместе с потоковой загрузкой зашифрованного файла;
- получение списка записей пользователя;
- получение записи пользователя;
- скачивание зашифрованного файла записи пользователя.
...
- удаление записи;
- CLI-клиент;
- клиентская криптография для вычисления `KEK`, работы с `DEK` и шифрования payload.
...
- перенести прикладные ошибки из `internal/domain/model` в `internal/usecase`, чтобы доменный слой не содержал ошибки сценариев.

## После MVP

Перспективные функциональные расширения:

- смена мастер-ключа без перешифрования payload записи: клиент расшифровывает `DEK` старым `KEK`, вычисляет новый `KEK` из нового мастер-ключа и заново шифрует тот же `DEK`;
- multipart upload для файлов большего размера;
