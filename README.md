...
- клиент может запросить состояние загрузки и список уже сохраненных частей, чтобы продолжить прерванную передачу;
- каждая часть загружается отдельно по своему номеру, а уже сохраненную часть можно не отправлять повторно;
- загрузка завершается только после получения непрерывного набора частей с суммарным размером, равным `encrypted_size`;
- незавершенную загрузку можно отменить, после чего файл получает неуспешный статус.

Состояние multipart-загрузки хранится на сервере в PostgreSQL: ID сессии, ID записи, ID файла, ключ объекта в S3-совместимом хранилище, ID сессии multipart-загрузки объектного хранилища, размер зашифрованного файла, размер части и список уже загруженных частей с `ETag`. Поэтому клиент может восстановить прерванную загрузку: получить статус по `upload_id`, пропустить уже сохраненные части и дозагрузить недостающие.

`encrypted_size` передается клиентом и используется сервером для проверки полноты загрузки. Во время приема части сервер считает фактический размер полученных чанков и сравнивает его с заявленным размером части. При завершении multipart-загрузки сервер проверяет непрерывность частей и их суммарный размер. При совпадении загрузка считается успешной, а приватная запись файла получает статус `uploaded`.
