...
- заменить использование текущего времени в тестах на фиксированные значения там, где время не является предметом проверки.
...
- переход от текущего интерактивного CLI на TUI: текущий CLI-режим достаточен для MVP, но ручная обработка терминального ввода остается хрупкой и может быть заменена полноценным терминальным интерфейсом после стабилизации основных сценариев; при редактировании записей TUI должен позволять изменять текущие значения прямо в полях ввода, без полного повторного ввода поля пользователем;
