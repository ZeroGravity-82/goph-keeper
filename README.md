...
В метаданных запрещено хранить секретные значения: пароли, коды восстановления, CVC/CVV, токены, приватные ключи и другую чувствительную информацию. Любые секретные данные должны храниться внутри зашифрованного payload приватной записи.
...
- смена мастер-ключа без перешифрования payload приватной записи: клиент расшифровывает `DEK` старым `KEK`, вычисляет новый `KEK` из нового мастер-ключа и заново шифрует тот же `DEK`;
