...
- создание и замена бинарной приватной записи через возобновляемую multipart-загрузку зашифрованного файла;
...
- создание приватной записи типа `binary` с клиентским шифрованием payload и файла через multipart-загрузку;
- обновление открытых метаданных приватной записи типа `binary` без повторной загрузки файла;
- замена файла приватной записи типа `binary` с клиентским шифрованием payload и файла через multipart-загрузку;
...
Зашифрованные значения хранятся в формате AES-GCM blob: сначала nonce, затем ciphertext. Такой формат используется для зашифрованного `DEK`, зашифрованного payload приватной записи и каждой зашифрованной части файла в объектном хранилище. `Nonce` не является секретом, но для каждого шифрования должен быть уникальным в рамках используемого ключа.
...
Для усложнения перебора мастер-ключа используется медленная функция вычисления ключа `Argon2id`. Ее текущие параметры:

- 3 прохода усложняют перебор мастер-ключа без чрезмерной задержки для CLI-клиента;
- 64 МиБ памяти заметно повышают стоимость перебора на GPU/ASIC, но остаются приемлемым значением для обычного пользовательского устройства;
...
Эти значения являются стартовыми параметрами. При необходимости их можно пересмотреть после замеров времени вычисления `KEK` на целевых устройствах.
...
При потере связи с сервером CLI-клиент переходит в режим чтения и уведомляет об этом пользователя. В этом режиме недоступны создание, изменение, удаление записей, смена мастер-ключа и скачивание файлов с сервера. Пользователь может работать со списком и полными данными записей, которые уже были загружены в память за текущий запуск приложения. Клиент не выполняет фоновую загрузку всех приватных записей и файлов: если запись или файл не были открыты до потери связи, они останутся недоступны до восстановления соединения с сервером. После успешного обновления списка записей клиент сообщает о восстановлении связи и возвращается в обычный режим.
...
1. Клиент использует версию приватной записи, полученную при последнем открытии записи или обновлении списка.
...
- payload каждой приватной записи шифруется отдельным `DEK`
...
Для учетных данных пользователя, метаданных и приватных записей заданы явные лимиты размера. Они защищают CLI-клиент и
gRPC-сервер от случайной или намеренной передачи слишком больших логинов, паролей, метаданных, полей payload и
зашифрованных данных.

| Данные | Ограничение                                                   | Где проверяется |
| --- |---------------------------------------------------------------| --- |
| Логин пользователя сервиса | 128 символов                                                  | клиент, сервер |
| Пароль пользователя сервиса | 256 символов                                                  | клиент, сервер |
| Мастер-ключ пользователя | 256 символов                                                  | клиент |
| Соль мастер-ключа | 16 байт                                                       | сервер |
| Зашифрованный верификатор мастер-ключа | 128 байт                                                      | сервер |
| Название записи | 128 символов                                                  | клиент, сервер |
| Описание записи | 1024 символа                                                  | клиент, сервер |
| Поле `text` текстовой записи | 256 КиБ                                                       | клиент |
| Логин записи типа `credential` | 128 символов                                                  | клиент |
| Пароль записи типа `credential` | 256 символов                                                  | клиент |
| Номер банковской карты | 16 цифр или 4 группы по 4 цифры, проверка корректности номера | клиент |
| Имя владельца карты | 32 символа, латинские буквы и пробелы                         | клиент |
| Срок действия карты | формат `ММ/ГГ`, месяц от `01` до `12`                         | клиент |
| CVC карты | 3 цифры                                                       | клиент |
| Имя файла бинарной записи | 255 символов                                                  | клиент |
| MIME-тип содержимого файла бинарной записи | 255 символов                                                  | клиент |
| Зашифрованный `DEK` приватной записи | 128 байт                                                      | сервер |
| Зашифрованный payload приватной записи | 1024 КиБ                                                      | сервер |
| Исходный файл бинарной записи до шифрования | 1024 МиБ                                                      | клиент |
| Зашифрованный файл бинарной записи | 1025 МиБ                                                      | сервер |
| Стандартная часть multipart-загрузки зашифрованного файла | не меньше 5 МиБ, если зашифрованный файл больше 5 МиБ          | сервер |

Мотивация ограничений:

- логин пользователя сервиса, пароль пользователя сервиса и мастер-ключ вводятся человеком вручную в CLI-клиенте. Слишком
  большие значения ухудшают UX, усложняют повторный ввод и не дают практической пользы для выбранных сценариев;
- соль мастер-ключа генерируется клиентом и используется при вычислении `KEK` через `Argon2id`. Фиксированная длина
  16 байт достаточна для уникального результата вычисления ключа и позволяет серверу отсекать некорректный формат;
- верификатор мастер-ключа нужен клиенту, чтобы проверить введенный мастер-ключ до работы с приватными записями. Сервер
  хранит только зашифрованную форму верификатора и не может проверить его содержимое, поэтому ограничивает размер входных
  зашифрованных данных. В текущем формате AES-256-GCM зашифрованный верификатор занимает 62 байта: 12 байт `nonce`, 34 байта
  зашифрованного проверочного текста и 16 байт тега аутентификации. Лимит 128 байт оставляет запас на формат и отсекает
  явно некорректные значения;
- название и описание записи являются открытыми метаданными и выводятся в списке записей в таблице ограниченной ширины.
  Поэтому они должны оставаться короткими и пригодными для быстрого просмотра;
- текстовая запись предназначена для заметок и текстовых секретов, а не для больших документов. Для больших документов и
  файлов предусмотрена бинарная запись;
- логин и пароль записи типа `credential` соответствуют обычным учетным данным внешних сервисов. Лимит пароля в 256
  символов покрывает длинные сгенерированные пароли и парольные фразы;
- поля банковской карты имеют естественные ограничения формата. Имя владельца физически ограничено размером банковской
  карты: обычно оно короче 20-21 символа, в редких случаях может быть около 25-26 символов, поэтому лимит 32 символа
  оставляет небольшой запас;
...
- зашифрованный `DEK` в текущем формате AES-256-GCM занимает 60 байт: 12 байт `nonce`, 32 байта зашифрованного `DEK` и
  16 байт тега аутентификации. Лимит 128 байт оставляет запас на формат и отсекает явно некорректные значения.
- лимит исходного файла 1024 МиБ является продуктовым ограничением, а не техническим пределом потокового шифрования или
  multipart-загрузки. Он нужен, чтобы ограничить объем хранимых пользовательских данных, стоимость хранения и последствия
  ошибочной или вредоносной загрузки слишком больших файлов;
- серверный лимит зашифрованного файла вычисляется из продуктового лимита исходного файла и служебных данных AES-GCM.
  Сам ciphertext имеет тот же размер, что исходный файл, но каждая зашифрованная часть добавляет 28 байт: 12 байт `nonce`
  и 16 байт тега аутентификации. При исходном файле 1024 МиБ и размере части 5 МиБ получается 205 частей, поэтому максимальный
  расчетный размер зашифрованного файла равен `1024 МиБ + 205 * 28 байт = 1 073 747 564 байта`. В коде серверный лимит
  округлен вверх до 1025 МиБ, чтобы не использовать менее читаемое значение в байтах;
- минимальный размер части multipart-загрузки выбран равным 5 МиБ, потому что это стандартное ограничение S3-совместимого
  API multipart-загрузки для всех частей, кроме последней.

Клиент проверяет отдельные поля payload до шифрования и показывает ошибку по конкретному полю. Размер зашифрованных данных клиент не валидирует: это входная валидация gRPC-сервера, потому что сервер должен защищать свою границу API от любого клиентского ПО. Сервер не видит незашифрованное содержимое payload, поэтому проверяет только размер зашифрованного payload и зашифрованного DEK. Лимит зашифрованного payload задан выше лимитов исходных полей, потому что перед шифрованием данные сериализуются в JSON: маршалинг добавляет служебную структуру и может экранировать отдельные символы. Запас до 1024 КиБ учитывает эти накладные расходы и неизвестный заранее состав пользовательского ввода. Если нужно сохранить большой текстовый документ, его следует сохранять как бинарную запись с файлом.
...
В объектное хранилище попадает только зашифрованное содержимое файла. Сервер не имеет доступа к исходному файлу и мастер-ключу пользователя.

Бинарная приватная запись создается отдельным сценарием multipart-загрузки, а не обычным сценарием создания записи. Клиент сначала отправляет метаданные записи и параметры зашифрованного файла: название, описание, зашифрованный `DEK`, зашифрованный payload записи, размер зашифрованного файла (`encrypted_size`) и размер части (`part_size`). Размер зашифрованного файла вычисляется заранее по размеру исходного файла и количеству шифруемых частей. Сервер создает запись со статусом файла `uploading` и возвращает `upload_id`.

Multipart-загрузка используется и для создания новой бинарной записи, и для замены файла существующей бинарной записи:

- сервер создает или подготавливает бинарную запись, запись с метаданными файла со статусом `uploading` и сессию multipart-загрузки;
- клиент может запросить состояние загрузки и список уже сохраненных частей, чтобы корректно обработать сетевой сбой в текущем CLI-сеансе;
- каждая часть загружается отдельно по своему номеру, а при потере ответа клиент может проверить, успел ли сервер принять текущую часть;
- загрузка завершается только после получения непрерывного набора частей с суммарным размером, равным `encrypted_size`, и проверки контрольной суммы зашифрованного файла;
- незавершенную загрузку можно отменить, после чего файл получает неуспешный статус.

Состояние multipart-загрузки хранится на сервере в PostgreSQL: ID сессии, ID записи, ID файла, ключ объекта в S3-совместимом хранилище, ID сессии multipart-загрузки объектного хранилища, размер зашифрованного файла, размер части и список уже загруженных частей с `ETag`. После успешного завершения загрузки сервер сохраняет SHA-256 зашифрованного файла в метаданных файла. Это позволяет клиенту проверить состояние загрузки при временной ошибке и не считать неудачной часть, которую сервер уже успел принять.

`encrypted_size` передается клиентом и используется сервером для проверки полноты загрузки. Во время приема части сервер считает фактический размер полученных чанков и сравнивает его с заявленным размером части. При завершении multipart-загрузки сервер проверяет непрерывность частей и их суммарный размер.

Для проверки целостности содержимого клиент во время загрузки считает SHA-256 от фактически отправляемого зашифрованного файла и передает контрольную сумму при завершении multipart-загрузки. После сборки объекта в S3-совместимом хранилище сервер потоково перечитывает зашифрованный файл, рассчитывает SHA-256 и сравнивает его со значением клиента. Если размер и контрольная сумма совпали, файл получает статус `uploaded`. Если контрольная сумма не совпала, файл получает статус `failed`.

`encrypted_size` отвечает только за полноту и размер передачи, а SHA-256 - за целостность содержимого. Это позволяет обнаружить не только неполную передачу, но и замену зашифрованного содержимого другим blob того же размера.

Открытые метаданные бинарной приватной записи, например название и описание, обновляются без повторной загрузки файла. Замена самого файла выполняется отдельным сценарием: клиент передает ID записи и ожидаемую версию, заново шифрует описание файла в `encrypted_payload`, потоково шифрует файл частями и передает новый зашифрованный файл на сервер по частям.

Клиент не читает исходный файл целиком в память при создании или замене бинарной записи. Он открывает файл, проверяет его размер, читает очередную часть, шифрует ее через AES-256-GCM и сразу отправляет как часть multipart-загрузки. Каждая зашифрованная часть имеет собственный nonce и authentication tag.

Для файлов размером от 1 МиБ CLI-клиент показывает прогресс загрузки в формате `Загрузка файла: 62 МиБ из 1024 МиБ, 6.1%`.

Сервер тоже не буферизует зашифрованный файл целиком в памяти: полученные чанки каждой multipart-части потоково передаются в объектное хранилище через `io.Reader`. Это снижает потребление памяти при загрузке файлов в пределах установленного лимита.
...
CLI-клиент показывает статус загрузки файла в списке записей и при просмотре бинарной приватной записи: `uploading`, `uploaded` или `failed`.

При скачивании сервер отдает зашифрованный файл gRPC-стримом чанками по 64 КиБ. Такой размер выбран как практичный компромисс: он не требует держать файл целиком в памяти, не создает чрезмерное количество gRPC-сообщений и достаточно мал для стабильной потоковой передачи.
...
Это означает, что значение по умолчанию может быть переопределено в конфигурационном файле, переменная окружения переопределяет значение из конфигурационного файла, а параметр командной строки имеет наивысший приоритет. Если переменная окружения задана, она считается явным значением даже при пустой строке. Пустое значение обязательного параметра не приводит к откату к нижестоящему источнику и считается ошибкой конфигурации.
...
Для локальной разработки в репозитории хранятся примеры конфигурационных файлов `config/server.local.example.yaml` для сервера и `config/client.local.example.yaml` для CLI-клиента. Они показывают полный набор поддерживаемых настроек и содержат заглушки вместо секретных значений. Реальные локальные файлы `config/server.local.yaml` и `config/client.local.yaml` создаются разработчиком вручную, не коммитятся и добавлены в `.gitignore`.

Перед запуском вне локального dev-окружения нужно заменить демонстрационные секреты:

- пароль PostgreSQL `userpassword` в `compose.yaml` и в `database_uri` серверной конфигурации;
- учетные данные MinIO `gophkeeper` / `gophkeeperpassword` в `compose.yaml`;
- `GOPHKEEPER_JWT_SECRET` - секрет подписи JWT;
- `GOPHKEEPER_FILE_STORAGE_ACCESS_KEY` и `GOPHKEEPER_FILE_STORAGE_SECRET_KEY` - ключи доступа сервера к объектному хранилищу.

Сервер позволяет настроить время жизни токенов через параметры `access_token_ttl` и `refresh_token_ttl`. Значения задаются в формате Go, например `15m` или `720h`. По умолчанию access-токен живет 15 минут, refresh-токен - 720 часов.

CLI-клиент поддерживает те же источники конфигурации, что и сервер: дефолтные значения, конфигурационный файл, переменные окружения и флаги командной строки. Для клиента обязательны параметры `grpc_address` и `ca_cert`; при локальном запуске их удобно указывать в рабочем конфигурационном файле `config/client.local.yaml`, созданном из примера.
...
Перед первым локальным запуском нужно создать рабочие конфигурационные файлы из примеров и заменить заглушки на значения своего окружения:

```bash
cp config/server.local.example.yaml config/server.local.yaml
cp config/client.local.example.yaml config/client.local.yaml
```
...
CLI-клиент может быть собран под Linux, macOS и Windows стандартной кросс-компиляцией Go через `GOOS` и `GOARCH`. В примерах ниже также передаются версия и дата сборки, чтобы они отображались в стартовом меню CLI-клиента.
...
GOOS=linux GOARCH=amd64 go build \
  -ldflags "-X main.buildVersion=dev -X main.buildDate=$(date -u +%Y-%m-%d)" \
  -o ./bin/gophkeeper-client-linux-amd64 ./cmd/client
```
...
GOOS=darwin GOARCH=amd64 go build \
  -ldflags "-X main.buildVersion=dev -X main.buildDate=$(date -u +%Y-%m-%d)" \
  -o ./bin/gophkeeper-client-darwin-amd64 ./cmd/client
```
...
GOOS=windows GOARCH=amd64 go build \
  -ldflags "-X main.buildVersion=dev -X main.buildDate=$(date -u +%Y-%m-%d)" \
  -o ./bin/gophkeeper-client-windows-amd64.exe ./cmd/client
```

## Надежность клиентских запросов

Для временных сетевых ошибок CLI-клиент использует короткие повторы с backoff для читающих унарных gRPC-запросов, например получения списка и чтения приватной записи. Клиент делает исходный запрос и до двух повторных попыток с задержками 100 мс и 200 мс. Изменяющие унарные запросы автоматически не повторяются, потому что клиент не может надежно определить, успел ли сервер применить операцию до сетевого сбоя. При multipart-загрузке файла клиент может повторить загрузку отдельной части: перед повтором он запрашивает статус сессии multipart-загрузки и пропускает часть, если сервер уже успел ее сохранить.
