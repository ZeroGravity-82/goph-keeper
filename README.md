...
7. Сервер одной транзакцией обновляет данные мастер-ключа пользователя и `encrypted_dek` всех активных приватных записей.
8. Если состав записей или версия хотя бы одной записи изменились во время смены мастер-ключа, сервер возвращает конфликт, а клиент просит повторить действие.
