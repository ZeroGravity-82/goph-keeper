...
Для локальной разработки в репозитории хранятся примеры конфигурационных файлов `config/server.local.yaml` для сервера и `config/client.local.yaml` для CLI-клиента. В них задаются демонстрационные настройки локального окружения, включая dev-значения для подключения к локальным PostgreSQL и MinIO. Секретные значения для запуска вне локального dev-окружения передаются отдельно через переменные окружения или внешнее хранилище секретов.
...
CLI-клиент поддерживает те же источники конфигурации, что и сервер: дефолтные значения, конфигурационный файл, переменные окружения и флаги командной строки. Для клиента обязательны параметры `grpc_address` и `ca_cert`; для локального запуска они явно указаны в `config/client.local.yaml`.
